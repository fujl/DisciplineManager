//
//  DMUserDetailRequester.m
//  DisciplineManager
//
//  Created by fujl-mac on 2017/7/26.
//  Copyright © 2017年 fujl-mac. All rights reserved.
//

#import "DMUserDetailRequester.h"
#import "DMUserModel.h"

@interface DMUserDetailRequester () <DMRequesterDelegate>

@end

@implementation DMUserDetailRequester

- (instancetype)init {
    self = [super init];
    if (self) {
        self.delegate = self;
    }
    return self;
}

#pragma mark - HlwyyRequesterDelegate

- (id)onDumpData:(NSDictionary *)jsonObject {
    NSDictionary *errData = [jsonObject objectForKey:@"errData"];
    DMUserModel *model = [[DMUserModel alloc] initWithDict:errData];
    return model;
}

- (id)onDumpErrorData:(NSDictionary *)jsonObject {
    return [jsonObject objectForKey:@"errMsg"];
}

- (NSString *)getChildrenUrl {
    return @"api/user.do";
}

/**
 *  放入请求的基本参数
 *
 *  @param params 参数容器，注意：里面已经放入基本的请求参数
 */
- (void)onPutParams:(NSMutableDictionary *)params {
    
}

- (BOOL)onPostJson {
    return NO;
}

@end

//
//  HMCMSDetailRequester.m
//  DisciplineManager
//
//  Created by fujl-mac on 2017/8/11.
//  Copyright © 2017年 fujl-mac. All rights reserved.
//

#import "DMCMSDetailRequester.h"
#import "DMCmsContentModel.h"

@interface DMCMSDetailRequester () <DMRequesterDelegate>

@end

@implementation DMCMSDetailRequester

- (instancetype)init {
    self = [super init];
    if (self) {
        self.delegate = self;
    }
    return self;
}

#pragma mark - HlwyyRequesterDelegate
- (id)onDumpData:(NSDictionary *)jsonObject {
    NSDictionary *errData = [jsonObject objectForKey:@"errData"];
    DMCmsContentModel *cmsInfo = [[DMCmsContentModel alloc] initWithDict:errData];
    return cmsInfo;
}

- (id)onDumpErrorData:(NSDictionary *)jsonObject {
    return [jsonObject objectForKey:@"errMsg"];
}

- (NSString *)getChildrenUrl {
    return @"api/cms/content/get.do";
}

/**
 *  放入请求的基本参数
 *
 *  @param params 参数容器，注意：里面已经放入基本的请求参数
 */
- (void)onPutParams:(NSMutableDictionary *)params {
    [params removeAllObjects];
    params[@"id"] = @(self.ccId);
    
}

- (BOOL)onPostJson {
    return NO;
}

@end

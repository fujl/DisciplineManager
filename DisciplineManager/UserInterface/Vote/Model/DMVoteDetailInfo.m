...
@implementation DMVoteDetailInfo

- (instancetype)initWithDict:(NSDictionary *)dict {
    self = [super init];
    if (self) {
        self.vId = parseStringFromObject([dict objectForKey:@"id"]);
        ...
        self.user = [[DMUserModel alloc] initWithDict:[dict objectForKey:@"user"]];//":null,
        ...
        NSArray *optionArray = parseArrayFromObject([dict objectForKey:@"options"]);
        for (NSDictionary *dic in optionArray) {
            DMVoteOptionInfo *optionInfo = [[DMVoteOptionInfo alloc] initWithDict:dic];
            [self.options addObject:optionInfo];
        }
